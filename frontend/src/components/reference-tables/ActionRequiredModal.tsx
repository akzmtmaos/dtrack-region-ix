...
interface ActionRequiredModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (data: { actionRequiredCode: string; actionRequired: string }) => void
  initialData?: { id: number; action_required: string } | null
}
...
const ActionRequiredModal: React.FC<ActionRequiredModalProps> = ({ 
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        actionRequiredCode: initialData.id?.toString() || '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      onSave({
        actionRequiredCode: formData.actionRequiredCode,
      ...
      >
        ...
        <div className="px-6 py-5" style={{ borderBottom: `1px solid ${borderColor}` }}>
          <h2 className="text-lg font-semibold mb-1" style={{ color: textPrimary }}>
            {initialData ? 'Edit Action Required' : 'Add New Action Required'}
          </h2>
          <p className="text-xs" style={{ color: textSecondary }}>
            {initialData 
              ? 'Update the action required information below.'
              : 'Fill in the action required details below. All required fields are marked with an asterisk.'
            }
          </p>
        </div>
