...
interface UserLevelsModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (data: { userLevelName: string }) => void
  initialData?: { id: number; user_level_name?: string } | null
}

const UserLevelsModal: React.FC<UserLevelsModalProps> = ({
  isOpen,
  onClose,
  onSave,
  initialData
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        userLevelName: initialData.user_level_name || ''
      })
    } else {
      setFormData({
        userLevelName: ''
  ...
  const validate = () => {
    ...
    if (!formData.userLevelName.trim()) {
      newErrors.userLevelName = 'User Level Name is required'
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      onSave({
        userLevelName: formData.userLevelName.trim()
      })
      setFormData({
        userLevelName: ''
      })
      ...
      >
        ...
        <div className="px-6 py-5" style={{ borderBottom: `1px solid ${borderColor}` }}>
          <h2 className="text-lg font-semibold mb-1" style={{ color: textPrimary }}>
            {initialData ? 'Edit User Level' : 'Add New User Level'}
          </h2>
          <p className="text-xs" style={{ color: textSecondary }}>
            {initialData
              ? 'Update the user level information below.'
              : 'Fill in the user level details below. All required fields are marked with an asterisk.'
            }
          </p>
        </div>
                ...
                <div className="flex-1">
                  <input
                    ...
                    style={{
                      ...
                      border: `1px solid ${errors.userLevelName ? '#ef4444' : inputBorder}`,
                      color: textPrimary
                    }}
                    onFocus={(e) => e.target.style.borderColor = '#3ecf8e'}
                    onBlur={(e) => e.target.style.borderColor = errors.userLevelName ? '#ef4444' : inputBorder}
                  />
                  {errors.userLevelName && (
                    <p className="mt-1 text-xs" style={{ color: '#ef4444' }}>{errors.userLevelName}</p>
                  )}
                </div>
              </div>
            </div>
...
export default UserLevelsModal
