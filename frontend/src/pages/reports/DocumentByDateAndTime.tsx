...
const DocumentByDateAndTime: React.FC = () => {
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Document By Date and Time - Report</title>
          <style>
            @media print {
              @page { margin: 1cm; }
        ...
        <body>
          ...
          <h1>Document By Date and Time - Report</h1>
          <table>
            <thead>
              <tr>
                <th>Doc Type</th>
  ...
  const handleExportToExcel = () => {
    // Create CSV content
    const headers = ['Doc Type', 'Subject', 'Releasing Officer', 'Originating Office', 'Date', 'Action Officer', 'Office', 'Action Taken', 'Doc. Control No.', 'Remarks', 'Status']
    ...
    link.setAttribute('download', `document_by_date_and_time_${new Date().toISOString().split('T')[0]}.csv`)
  ...
  const handleExportToWord = () => {
    ...
    link.setAttribute('href', url)
    link.setAttribute('download', `document_by_date_and_time_${new Date().toISOString().split('T')[0]}.doc`)
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
...
export default DocumentByDateAndTime