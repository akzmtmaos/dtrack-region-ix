...
const DocumentByActionOfficer: React.FC = () => {
    ...
    const fetchDocuments = async () => {
      ...
      if (res.success && Array.isArray(res.data)) {
        const mapped: Document[] = (res.data as any[]).map((row: any) => ({
          ...
          remarks: row.remarks ?? '',
          originatingOffice: (row.internalOriginatingOffice || row.externalOriginatingOffice) ?? '',
        }))
        setDocuments(mapped)
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Document By Action Officer - Report</title>
          <style>
            @media print {
              @page { margin: 1cm; }
        ...
        <body>
          ...
          <h1>Document By Action Officer - Report</h1>
          <table>
            <thead>
              <tr>
  ...
  const handleExportToExcel = () => {
    // Create CSV content
    const headers = ['Action Officer', 'Office', 'Doc Type', 'Subject', 'Date', 'Action Taken', 'Doc. Control No.', 'Remarks', 'Status']
    ...
    link.setAttribute('download', `document_by_action_officer_${new Date().toISOString().split('T')[0]}.csv`)
  ...
  const handleExportToWord = () => {
    ...
    link.setAttribute('href', url)
    link.setAttribute('download', `document_by_action_officer_${new Date().toISOString().split('T')[0]}.doc`)
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    ...
    <div className="pt-4 pb-8">
      ...
      >
        ...
        <tbody className={`divide-y ${
          ...
          {loading ? (
            <tr>
              <td colSpan={9} className={`px-6 py-8 text-center text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                Loading...
              </td>
            </tr>
          ) : documents.length === 0 ? (
            <tr>
              <td colSpan={9} className={`px-6 py-8 text-center text-sm ${theme === 'dark' ? 'text-white' : 'text-gray-500'}`}>
                No documents found
              </td>
            </tr>
          ) : (
            documents.map((doc) => (
              <tr key={doc.id} className={`transition-colors ${theme === 'dark' ? 'hover:bg-dark-hover' : 'hover:bg-gray-50'}`}>
                <td className={`px-6 py-4 whitespace-nowrap text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>{doc.recipient || '—'}</td>
