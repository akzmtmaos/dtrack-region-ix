...
const ActionRequired: React.FC = () => {
  ...
  const {
    ...
    searchFilter: (item, query) => {
      ...
      const actionRequired = item.action_required?.toLowerCase() || ''
      return idString.includes(searchLower) || actionRequired.includes(searchLower)
    }
  })
  ...
  const handleDelete = (item: ActionRequiredItem) => {
    setDeleteType('single')
    setDeleteId(item.id)
    setDeleteItemName(item.action_required)
    setIsDeleteModalOpen(true)
  }
  ...
  const confirmSingleDelete = async () => {
    ...
    setError(null)
    try {
      const response = await apiService.deleteActionRequired(deleteId)
      if (response.success) {
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Action Required - Reference Table</title>
          <style>
            @media print {
              @page { margin: 1cm; }
        ...
        <body>
          ...
          <h1>Action Required - Reference Table</h1>
          <table>
            <thead>
              <tr>
                <th>Action Required ID</th>
                <th>Action Required</th>
              </tr>
            </thead>
            <tbody>
              ${filteredItems.length === 0 
                ? '<tr><td colspan="2" class="no-data">No items found</td></tr>'
  ...
  const handleExportToExcel = () => {
    // Create CSV content
    const headers = ['Action Required ID', 'Action Required']
    ...
    link.setAttribute('download', `action_required_${new Date().toISOString().split('T')[0]}.csv`)
      ...
      >
        <thead className={theme === 'dark' ? 'bg-dark-hover/60' : 'bg-gray-50'}>
          <tr>
            ...
            <th className={`px-6 py-2 whitespace-nowrap text-left text-xs font-medium uppercase tracking-wider ${
