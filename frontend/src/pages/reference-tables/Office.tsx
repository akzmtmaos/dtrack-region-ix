...
const Office: React.FC = () => {
  ...
  const confirmBulkDelete = async () => {
    ...
    setError(null)
    try {
      const response = await apiService.bulkDeleteOffice(selectedItems)
      if (response.success) {
  ...
  const handleSave = async (data: { office: string; region: string; shortName: string; headOffice: string }) => {
    setLoading(true)
    setError(null)
    try {
      if (editingItem) {
        // Update existing item
        const response = await apiService.updateOffice(editingItem.id, data)
        if (response.success && response.data) {
          // Map database response to frontend format
          const updatedItem = {
            id: response.data.id,
            office: response.data.office || data.office,
            region: response.data.region || data.region,
            shortName: response.data.short_name || data.shortName,
            headOffice: response.data.head_office || data.headOffice
          }
        ...
        // Add new item
        const response = await apiService.createOffice(data)
        if (response.success && response.data) {
          // Map database response to frontend format
          const newItem = {
            id: response.data.id,
  ...
  const handleDelete = (item: OfficeItem) => {
    setDeleteType('single')
    setDeleteId(item.id)
    setDeleteItemName(item.office)
    setIsDeleteModalOpen(true)
  }
  ...
  const confirmSingleDelete = async () => {
    ...
    setError(null)
    try {
      const response = await apiService.deleteOffice(deleteId)
      if (response.success) {
  ...
  return (
    <div className="pt-4 pb-8">
      ...
      >
        ...
        <tbody className={`divide-y ${
          ...
          {loading && items.length === 0 ? (
            <tr>
              <td colSpan={7} className={`px-4 py-2 text-center text-xs ${
      ...
      <OfficeModal
        ...
        onSave={handleSave}
        initialData={editingItem ? { id: editingItem.id, office: editingItem.office, region: editingItem.region, short_name: editingItem.shortName, head_office: editingItem.headOffice } : null}
      />
