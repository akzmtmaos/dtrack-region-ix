...
const ActionTaken: React.FC = () => {
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Action Taken - Reference Table</title>
          <style>
            @media print {
              @page { margin: 1cm; }
        ...
        <body>
          ...
          <h1>Action Taken - Reference Table</h1>
          <table>
            <thead>
              <tr>
  ...
  const handleExportToExcel = () => {
    // Create CSV content
    const headers = ['Action Taken ID', 'Action Taken']
    ...
    link.setAttribute('download', `action_taken_${new Date().toISOString().split('T')[0]}.csv`)
