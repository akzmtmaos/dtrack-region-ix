...
const DocumentType: React.FC = () => {
  ...
  const handleDelete = (item: DocumentTypeItem) => {
    ...
    setDeleteId(item.id)
    setDeleteItemName(item.documentType)
    setIsDeleteModalOpen(true)
  }
  ...
  const confirmSingleDelete = async () => {
    ...
    setError(null)
    try {
      const response = await apiService.deleteDocumentType(deleteId)
      if (response.success) {
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Document Type - Reference Table</title>
          <style>
            @media print {
              @page { margin: 1cm; }
        ...
        <body>
          ...
          <h1>Document Type - Reference Table</h1>
          <table>
            <thead>
              <tr>
  ...
  const handleExportToExcel = () => {
    // Create CSV content
    const headers = ['Document Type ID', 'Document Type']
    ...
    link.setAttribute('download', `document_type_${new Date().toISOString().split('T')[0]}.csv`)
