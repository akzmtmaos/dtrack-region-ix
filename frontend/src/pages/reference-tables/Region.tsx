...
const Region: React.FC = () => {
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Region - Reference Table</title>
          <style>
            @media print {
              @page { margin: 1cm; }
              ...
              ${filteredItems.length === 0
                ...
                : filteredItems.map(item => `
                  <tr>
                    ...
                    <td>${item.nscb_code || '—'}</td>
                    <td>${item.nscb_name || '—'}</td>
                    <td>${item.added_by || '—'}</td>
  ...
  const handleExportToExcel = () => {
    // Create CSV content
    const headers = ['Region ID', 'Region Name', 'NSCB Code', 'NSCB Name', 'Added By', 'Date Updated', 'Status']
    const csvRows = [headers.join(',')]

    filteredItems.forEach(item => {
      const row = [
        `\"${String(item.id).padStart(5, '0').replace(/\"/g, '\"\"')}\"`,
        `\"${(item.region_name || '').replace(/\"/g, '\"\"')}\"`,
        `\"${(item.nscb_code || '').replace(/\"/g, '\"\"')}\"`,
        `\"${(item.nscb_name || '').replace(/\"/g, '\"\"')}\"`,
        `\"${(item.added_by || '').replace(/\"/g, '\"\"')}\"`,
        `\"${(item.date_updated ? new Date(item.date_updated).toLocaleDateString() : '').replace(/\"/g, '\"\"')}\"`,
    ...
    link.setAttribute('download', `region_${new Date().toISOString().split('T')[0]}.csv`)
