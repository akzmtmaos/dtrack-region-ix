...
class DocumentSourceController extends Controller
{
    ...
    private function canAccessDocumentRow(?string $employeeCode, DocumentSource $row): bool
    {
    ...
    private function canDownloadAttachment(?string $employeeCode, DocumentSource $row): bool
    {
        ...
        $uid = trim((string) ($row->userid ?? ''));
        $holder = $cust !== '' ? $cust : $uid;
        if ($holder !== '' && EmployeeMatchService::identityMatches($employeeCode, $holder)) {
            return true;
        }
        if ($uid !== '' && EmployeeMatchService::identityMatches($employeeCode, $uid)) {
            return true;
        }
        $dest = DocumentDestination::query()
            ...
            ->get();
        foreach ($dest as $d) {
            if (EmployeeMatchService::identityMatches($employeeCode, $d->employee_action_officer)) {
                return true;
            }
        }
        $candidates = EmployeeMatchService::matchCandidates($employeeCode);
        $internal = (string) ($row->internal_originating_employee ?? '');
        $external = (string) ($row->external_originating_employee ?? '');
        if (EmployeeMatchService::originatingMatches($internal, $candidates, $employeeCode)) {
            return true;
        }
        if (EmployeeMatchService::originatingMatches($external, $candidates, $employeeCode)) {
            return true;
        }
    ...
    public function update(Request $request, int $item_id): JsonResponse
    {
        ...
        $ec = $this->employeeScope($request);
        if ($ec && ! $this->canAccessDocumentRow($ec, $row)) {
    ...
    public function bulkSoftDelete(Request $request): JsonResponse
    {
        ...
        foreach ($clean as $iid) {
            ...
            if ($row && (! $ec || $this->canAccessDocumentRow($ec, $row))) {
                $allowed[] = $iid;
            }
        }
        if ($allowed === []) {
